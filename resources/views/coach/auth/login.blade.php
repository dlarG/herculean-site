...
<body class="antialiased flex items-center justify-center min-h-screen px-5">
    <div class="w-full max-w-sm">
        ...
        <h1 class="display text-3xl text-center mb-8" style="color: var(--gold);">COACH LOGIN</h1>
        ...
        <form method="POST" action="{{ route('coach.login') }}" class="space-y-4">
            @csrf
            <div>
                <label class="block text-sm font-medium mb-1.5">Username</label>
                <input type="text" name="username" required autofocus class="field w-full rounded px-3 py-2.5" value="{{ old('username') }}">
            </div>
            <div>
